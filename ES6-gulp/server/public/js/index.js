...
var _base = __webpack_require__(333);
	...
	try {
		for (var _iterator = Reflect.ownKeys(source)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			...
			if (key !== 'constructor' && key !== 'prototype' && key !== 'name') {
				var desc = Object.getOwnPropertyDescriptor(source, key); //返回指定对象所有自身属性（非继承属性）的描述对象（明文）
				Object.defineProperty(target, key, desc); //给目标对象增加一个属性，属性名为key，内容为desc
			}
		}
	} catch (err) {
    ...
    handle：截止后的回调函数*/
		value: function countdown(end, update, handle) {
			...
			if (now - end > 0) {
				...
				var last_time = end - now; //计算剩余时间，单位ms
				var px_d = 1000 * 60 * 60 * 24; //将一天转换为ms
				var px_h = 1000 * 60 * 60; //将小时转换为ms
				var px_m = 1000 * 60; //将分钟转换为ms
				var px_s = 1000; //将秒转换为ms
...
var Base = function () {
   ...
   */
		...
		value: function toggleCodeActive(e) {
			var self = this;
			var $cur = (0, _jquery2.default)(e.currentTarget); //获取当前选中的对象的dom结构
   ...
   */
		...
		value: function changePlayNav(e) {
			...
			(0, _jquery2.default)('#zx_sm span').html(self.play_list.get(self.cur_play).tip); //将当前选中的提示信息tip显示
			(0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); //切换玩法后会将上一次选中的号码都清空
			self.getCount();
		}
   ...
   */
		...
		value: function assistHandle(e) {
			...
			var self = this;
			var $cur = (0, _jquery2.default)(e.currentTarget);
			var index = $cur.index(); // 返回点击的是第几个按钮
			(0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); //先清楚所有被选的状态
			if (index === 0) {
				//全选
   ...
   */
		...
		value: function addCode() {
			var self = this;
			var $active = (0, _jquery2.default)('.boll-list .btn-boll-active').text().match(/\d{2}/g); //将选中的号码2个为一组分割
			var active = $active ? $active.length : 0; //计算数组的长度
			var count = self.computeCount(active, self.cur_play); //算出注数
			if (count) {
   ...
   */
		...
		value: function getRandom(num) {
			var arr = [],
			    index = void 0;
			var number = Array.from(this.number); //将set集合转成真正的数组
			while (num--) {
...
var Calculate = function () {
   ...
   */
		value: function computeCount(active, play_name) {
			var count = 0; //当前默认注数为0
			var exist = this.play_list.has(play_name); //判断是否有play_name玩法
			var arr = new Array(active).fill('0'); //初始化一个长度为active的数组，每个元素为0
   ...
   */
		...
		value: function computeBonus(active, play_name) {
			var play = play_name.split(''); //当前玩法标识的数组
			var self = this;
				...
				if (size === arrLen) {
					...
					for (var i = 0; i < arrLen; i++) {
						...
						if (size === 1) {
							...
							newArr.splice(0, i + 1); //从当前数组的第0个元素删除i+1个元素
							f(newArr, size - 1, newResult); //newArr：当前需被组合的剩余部分，newResult：当前已经被组合的部分
						}
					}
				}
			})(arr, size, []);
		...
		value: function getOmit(issue) {
			...
			return new Promise(function (resolve, reject) {
				_jquery2.default.ajax({
					...
					error: function error(XMLHttpRequest, errormessage) {
